...
class TextClassifierTrainer:
    ...
    def __init__(self, dataset_name : str, topic : str, learning_rate : float, epochs : int):
    ...
    def create_dataset(self):
        ...
        tokenized_data = tokenizer(huggingface_data['text'], max_length=512, padding=True, truncation=True)
        split_tokenized_hugginface_data = tokenized_data.train_test_split(test_size=0.10)
        print(split_tokenized_hugginface_data)
        
        return tokenizer, split_tokenized_hugginface_data, eval_data
    
    def train_model(self, dataset, tokenizer):
        ...
        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=dataset["train"],#.select(range(200)),  # can limit our data input with select
...
for topic in Topics:
    ...
    tct = TextClassifierTrainer(dataset_name, topic, learning_rate, epochs)
    tokenizer, dataset, eval_data = tct.create_dataset()
    trainer = tct.train_model(dataset, tokenizer)
